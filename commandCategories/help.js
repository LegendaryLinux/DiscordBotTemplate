...
const { verifyIsAdmin, verifyModeratorRole } = require('../lib');
  ...
  commands: [
    {
      ...
      aliases: ['commands'],
      usage: `\`${config.commandPrefix}help [command]\``,
      minimumRole: null,
      adminOnly: false,
      guildOnly: true,
      execute(message, args) {
        ...
        if (!command) {
          return message.channel.send(`I don't know that command. Use \`${config.commandPrefix}help\` for more info.`)
            .catch((error) => {
                errorHandlers.dmErrorHandler(error, message);
              }
            );
